...
- link:library.html#lib.loadFile[loadFile]: load a single data file and return it as an attribute set
- link:library.html#lib.loadDir[loadDir]: to load all the files in a directory and return them as a list of attributes sets (non-recursive)
...
Then the list of posts can be fetched with link:library.html#lib.loadDir[loadDir] by passing the `renderDrafts` parameter.
...
For example, link:library.html#lib.mkPageList[mkPageList] can generate pages from a list of loaded data.
