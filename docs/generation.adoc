...
Most sites use link:library.html#lib.mkSitePackage[`mkSitePackage`], which loads themes, builds `pageList`, and calls `mkSite`.

For manual control, prepare a `pageList` and call link:library.html#lib.mkSite[`mkSite`] directly.
...
site = lib.mkSite { inherit files pageList; } # <2>
----

<1> Convert the pages attribute set to a list of pages.
<2> Generate the site.

It is possible to set default attributes to every page set during the `pages` attribute by using the `default` argument of the link:library.html#lib.pagesToList[pagesToList] function.
