...
Most sites use link:library.html#lib.generation.mkSitePackage[`mkSitePackage`], which loads themes, builds `pageList`, and calls `mkSite`.

For manual control, prepare a `pageList` and call link:library.html#lib.generation.mkSite[`mkSite`] directly.
...
site = lib.generation.mkSite { inherit files pageList; } # <2>
----
