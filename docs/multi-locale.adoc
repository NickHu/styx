...
NOTE: The former `localesToPageList` helper was removed. Combine each locale's pages with `lib.generation.pagesToList` and `lib.concatLists` (or build `pageList` manually) instead.
...
-----------------------------------------------------------------------------*/
{ pkgs, styxlib, styxthemes ? {}, extraConf ? {} }: rec {
...
-----------------------------------------------------------------------------*/
  ...
  */
  pageList = lib.concatLists (lib.mapAttrsToList (
    name: locale:
    lib.generation.pagesToList {
      pages = lib.mapAttrs (n: p: p // { path = locale.prefix + p.path; }) locale.pages;
      default = { layout = locale.env.templates.layout; };
    }
  ) locales);                                              <11>
...
<2> Put locale independent theme data to the main scope. `env` is exported to the main namespace so generated site documentation can inspect the loaded themes.
  ...
  */
  themesData = styxlib.mapAttrs (name: value:
    styxlib.themes.load {
      lib = styxlib;
      inherit themes;
      config = [ ./conf."${name}".nix extraConf ];    <1>
      env = value;
    }
  ) locales;
----
