...
NOTE: The former `localesToPageList` helper was removed. Combine each locale's pages with `lib.pagesToList` and `lib.concatLists` (or build `pageList` manually) instead.
