...
Split pages can be generated with link:library.html#lib.pages.mkSplit[mkSplit].

[source, nix]
.mkSplit example
----
pages.archives = lib.pages.mkSplit {
  basePath     = "/archives";
...
For a directory of posts or other content, link:library.html#lib.pages.mkPageList[mkPageList] generates one page per data file.

[source, nix]
.Generating pages from loaded data
----
pages.posts = lib.pages.mkPageList {
  data     = data.posts;
