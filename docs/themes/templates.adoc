...
This pattern of updating the page set `content` attribute and returning page set is so common that there is a link:library.html#lib.normalTemplate[normalTemplate] function to abstract it.
...
`conf`, `lib`, and `templates` are set automatically when themes load. In `mkSitePackage`, pass extra values through `extraEnv` or reference them from the `body` function.
