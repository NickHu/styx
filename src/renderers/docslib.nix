...
in rec {
  ...
  */
  ...
  mkConfDoc = id: path: conf: ''
    ...
    ${l.optionalString (conf ? default) ''
      ...
      [source, nix]
      ----
      ${styxlib.utils.prettyNix conf.default}
      ----
    ''}
    ${l.optionalString (conf ? example) ''
      ...
      [source, nix]
      ----
      ${styxlib.utils.prettyNix conf.example}
      ----
  ...
  in
    ...
    + l.concatStringsSep "\n" (map (
        arg: let
          ...
          default = l.optionalString (arg ? default) "Optional, defaults to `${styxlib.utils.prettyNix arg.default}`.";
        in
  ...
  mkTemplateExample = example: ''
    ...
    ${l.optionalString (example ? code) (
      ...
      ''
        ...
        .Result
        ----
        ${styxlib.utils.prettyNix example.code}
        ----
      ''
    )}
  ...
  */
  ...
  mkFunctionArgs = args: let
    ...
    isMultiline = arg: let
      arg' =
        if styxlib.utils.is "literalExpression" arg
        then arg.text
        else arg;
    in
      if l.isString arg'
        ...
        arg: let
          ...
          default =
            ...
            (
              ...
              then "Optional, defaults to:\n\n[source, nix]\n----\n${styxlib.utils.prettyNix arg.default}\n----\n"
              else "Optional, defaults to `${styxlib.utils.prettyNix arg.default}`."
            );
          example =
            l.optionalString (arg ? example)
            (
              if isMultiline arg.example
              then "Example:\n\n[source, nix]\n----\n${styxlib.utils.prettyNix arg.example}\n----\n"
              else "Example: `${styxlib.utils.prettyNix arg.example}`."
            );
        in
    ...
    ''
      ...
      ${
        l.optionalString (example ? code) ''
          ...
          .Result
          ----
          ${styxlib.utils.prettyNix (example.displayCode example.code)}
          ----
        ''
      }
