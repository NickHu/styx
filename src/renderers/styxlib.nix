...
let
  ...
  styxlib =
    ...
    // {
      inherit markup;
      inherit (build.data) loadFile loadDir markdownToHtml asciidocToHtml;
      inherit (build.pages) mkPageList mkSplit mkPages mkSplitPagePath;
      inherit (build.template)
        ...
        parseDate
        ;
      inherit (build.generation) generatePage mkSite pagesToList mkSitePackage;
